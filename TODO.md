...
- [x] Ajouter une suite de tests frontend et Playwright versionnée : recette vide, calcul valide/invalide, changement de méthode d'eau, import malformé, sauvegarde/réouverture, duplication, suppression et navigation clavier aux largeurs 360, 768 et 1280 px. — 74 tests unitaires `vitest` (`src/**/*.test.ts`, logique pure : décimaux, état de l'éditeur, validation d'import, garde d'erreur IPC) + 16 tests Playwright (`tests/e2e/*.spec.ts`) couvrant exactement les scénarios listés, IPC Tauri simulé via `tests/e2e/support/mockTauri.ts` (mêmes commandes que `src-tauri`, formules simplifiées en flottant JS — la correction des formules reste garantie par les 38 tests Rust).
- [x] Ajouter les tests frontend et Playwright à la CI, avec capture et trace en cas d'échec. — intégrés au job `quality` de `.github/workflows/ci.yml` (`npm run test:unit`, `npm run typecheck:e2e`, `npm run test:e2e`) ; rapport Playwright (captures + traces) publié comme artefact CI en cas d'échec.
...
- [x] Faire exécuter localement `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings` et `cargo test --workspace` sur une machine équipée de Rust, en complément du résultat CI. — exécuté à répétition sur cette machine tout au long des changements de cette section (38 tests, 0 avertissement clippy).
...
- [x] Import/export JSON — `recipeFile.ts` via `@tauri-apps/plugin-dialog` + deux commandes Rust dédiées (`lire_fichier_recette`/`ecrire_fichier_recette`, plus de `plugin-fs`, voir section sécurité ci-dessus) ; fichier auto-porteur (embarque la définition des ingrédients personnalisés) ; validation structurelle et syntaxique complète du fichier importé avant usage. Vérifié en conditions réelles (export puis réimport dans l'app) et couvert par des tests unitaires et Playwright.
